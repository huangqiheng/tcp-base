...
class TCPBase extends Base {
  ...
  bindingCreateConnection(port, host, options) {
	var SOL_SOCKET = 1;
	var SO_BINDTODEVICE = 25;
	var current = ffi.Library(null, {
		'setsockopt': ['int', ['int', 'int', 'int', 'string', 'int']]
	});
	var ourInterface = iface[this.options.localAddress];
	var ourIp = this.options.localAddress;

	var socket;
	socket = new net.Socket({handle: net._createServerHandle(ourIp)});
	var iface_opt = ourInterface;
	var r = current.setsockopt(socket._handle.fd, SOL_SOCKET, SO_BINDTODEVICE, iface_opt, 6);
	if (r === -1)
  ...
  _connect(done) {
	...
	*/
    ...
    const socket = this._socket = this.bindingCreateConnection(this.options);
